...
""".strip()
		...
		@observe(name="read_long_content", as_type="generation")
...
Context: {context}"""
				...
				result =  [term.strip() for term in response.completion.strip().split('\n') if term.strip()][:5]
			...
			if params is not None:
				...
				with span_context:
					try:
						...
						# with langfuse.start_as_current_observation(name=action_name, input={'action': action_name,'params': params,}, as_type="tool") as obs:
						# 	obs.update(
						# 		output=result
						# 	)
