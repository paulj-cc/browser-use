...
class ChatOpenAI(BaseChatModel):
	...
	async def ainvoke(
		...
		"""
		model_params_for_log = {k: v for k, v in {
		...
		with langfuse.start_as_current_observation(
			name=f"ainvoke/{self.model}",
			as_type="generation",
			input=[m.__dict__ if hasattr(m, '__dict__') else str(m) for m in messages],
			model=str(self.model),
			model_parameters=model_params_for_log,
		) as obs:
				...
				if output_format is None:
					...
					obs.update(
						...
						usage_details={
							...
							"cache_read_input_tokens": usage.prompt_cached_tokens if usage else 0,
							# total is omitted — Langfuse derives it from input + cache_read_input_tokens + output
						},
					)
